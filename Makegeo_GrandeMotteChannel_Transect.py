...
using QGIS. For each transect we add a channel footprint considering 3 possible shapes for the initial tunnel: rectangle, rectangle + half-circle, ovoide. The channel is the shape of the tunnel after
incision of ice by water down to the bedrock.
...
###We define a function to interpolate bottom corners of channel on bed DEM
    ...
    for Shape in Shapes:
        ...
        for k,(Width,Width_Name) in enumerate(zip(Widths, Width_Names)):
            ...
            if Shape == 'Ovoide':
                ...
                file_name_output = 'GrandeMotte_Channel_{}_{}.geo'.format(Transect, Shape)
            else:
                file_name_output='GrandeMotte_Channel_{}_{}_W{}cm.geo'.format(Transect,Shape,Width_Name)
            geo = open(pathroot_mycode.joinpath(file_name_output), 'w')
            geo.write('// This a a geo file created using the python script Makegeo_GrandeMotteChannel_Transect.py // \n')
            ...
            ###...continu with bottom nodes on the right side of channel
            ...
            ###Then we must split the DEM_bed to keep only the part located on the right side of the channel
            ...
            ###Now write the point corresponding to the bottom right corner of the channel and store index
            np = np + 1
            ...
            ###Now write the points corresponding to the roof of the channel
            ####For the rectangle and half circle case, only a few points are required
            if Shape == 'Rectangle' or Shape == 'Circle':
                np = np + 1
                ...
                if Shape == 'Circle':
                    np = np + 1
                    ChannelRoofCenterPoint_Index = np
            ...
            ####For the ovoid shape more points are required
                ...
                ChannelRoofRightCornerPoint_Index = np+1
                ...
                ChannelRoofLeftCornerPoint_Index = np
            ###...continu with bottom nodes on the left side of channel
            ###First bottom left corner of tunnel must be projected on BedDEM
            if Shape == 'Rectangle' or Shape == 'Circle':
            ...
            ###Now write the point corresponding to the bottom left corner of the channel and store index
            np = np + 1
            ...
            if spline:
                ...
                ###The spline/circle corresponding to the channel roof
                if Shape == 'Rectangle':
                    geo.write('Spline(5) = {')
                    geo.write('{},'.format(ChannelRoofRightCornerPoint_Index))
                    ...
                    geo.write('Circle(5) = {')
                    geo.write('{},'.format(ChannelRoofRightCornerPoint_Index))
                    geo.write('{},'.format(ChannelRoofCenterPoint_Index))
                    geo.write('{}'.format(ChannelRoofLeftCornerPoint_Index))
                    geo.write('}; \n')
                elif Shape == 'Ovoide':
                    geo.write('Spline(5) = {')
                    for j in range(ChannelRoofRightCornerPoint_Index,ChannelRoofLeftCornerPoint_Index):
                        geo.write('{},'.format(j))
                    geo.write('{}'.format(ChannelRoofLeftCornerPoint_Index) + '}; \n')
