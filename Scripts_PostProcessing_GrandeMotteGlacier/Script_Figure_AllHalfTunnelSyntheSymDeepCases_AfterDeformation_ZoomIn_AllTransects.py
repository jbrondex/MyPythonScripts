...
Description:
------------
This script is used to produce figure showing deformation of the deepest tunnel over time. Beware that the tunnel cases (init, incised straight walls,
...
if __name__ == "__main__":
    ...
    BottomTunnel_altitudes = [2785.5,2785,2784.5,2784,2783.5,2783.12]
    Bed_altitudes = [2784.75, 2784.75, 2779.53, 2774.56, 2773.12, 2774.82]
    #### Cases before/after incision
    Cases=['SlidArg', 'NoSlid']#, 'Channel_NoSlid']
    ...
    for Case in Cases: ###One Fig per case
        ...
        for k, (Transect, Bed_altitude) in enumerate(zip(Transect_Names,Bed_altitudes)):
            ...
            if Case == 'SlidArg':
                filename_output = 'RunTransient_Closure/RunTransient_{0}_Ovoide/ScalarOutput/TunnelOutput_HalfTunnelSyntheSym_Deep_{0}_Ovo_SlidArg_60StepPerY_.dat'.format(Transect)
            elif Case == 'NoSlid':
                filename_output = 'RunTransient_Closure/RunTransient_{0}_Ovoide/ScalarOutput/TunnelOutput_HalfTunnelSyntheSym_Deep_{0}_Ovo_NoSlid_60StepPerY_.dat'.format(Transect)
