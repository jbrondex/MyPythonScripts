...
if __name__ == "__main__":
 ...
 #   Timestep_size = 0.01/365 ### In years !!
    ...
    OutputOfPlot = 40/(Timestep_size*OutputInterval*365) ###At which output (count) do we want to make the plot (correspond to the day of simu)
    TimeOfPlot = (1+(OutputOfPlot-1)*OutputInterval)*Timestep_size ###Physical time (in years !) at which plot is made
    ...
    Marker_Size = 26 ### Size of markers to show where vertical profiles of variables vs time are plotted
    ...
    Df_wholebed=Df_wholebed[Df_wholebed['Count']==OutputOfPlot]
    Df_wholebed=Df_wholebed.copy()
    ...
    Chi = Interpolate_field(Df_wholebed,'Chi',Xbed,Ybed)
    #############################################
    ###     PLOT MAPS OF VARIABLE OF INTEREST ###
    #############################################
    ###~~~~~~~FIG 1 : Normal stress against bed ~~~~~~~~~~~~~~~
    ###Prepare the subplot
    ...
    #shading
    clevs=np.arange(-3.0, 3.01, 0.05)   ## cbar for shading
    ...
    cbar.set_ticks(np.arange(-3, 3.01, 1.0))
    ...
    ###Force x and y axis to same scale
    ax.set_aspect('equal', adjustable='box')
    ax.set_title('Cavitation', fontsize=21, weight='bold')
    #shading
    ...
    #CS1 = ax.pcolormesh(Xbed / 1000, Ybed / 1000, 1-theta, cmap=cmap, shading='auto', vmin=vmin, vmax=vmax, alpha=0.6, zorder=1)
    ax.plot(xc/1000, yc/1000 , color='k', linewidth=2)
    ###Plot stars where we plot variable versus time
    for xstar,ystar,colstar in zip(xplot,yplot,colplot):
        ax.plot(xstar/1000, ystar/1000, color=colstar, marker='*', markersize=Marker_Size, markeredgecolor='black', markeredgewidth=1.2)
    ...
    ###Force x and y axis to same scale
    ax.set_aspect('equal', adjustable='box')
    ax.set_title('Damaging areas', fontsize=21, weight='bold')
    #shading
    clevs=np.arange(0.0, 6.1, 0.1)   ## cbar for shading
    vmin, vmax = clevs[0], clevs[-1]
    Colormap = 'inferno'
    cmap = cmx.get_cmap(Colormap)
    #colorbar
    ...
    ax.imshow(ortho_crop, extent=extent_km, origin='upper', cmap='gray', zorder=0)
    ###Fills up the map with colors for SigmaEq
    # CS1 = ax.contourf(Xbed/1000, Ybed/1000, Chi, levels=clevs, cmap=cmap, extend='both', alpha=0.7, antialiased=True)
    CS1 = ax.pcolormesh(Xbed / 1000, Ybed / 1000, Chi,  cmap=cmap, shading='auto', vmin=vmin, vmax =vmax ,alpha=0.6,zorder=1)
    ax.plot(xc/1000, yc/1000 , color='k', linewidth=2)
    ###Plot stars where we plot variable versus time
    for xstar,ystar,colstar in zip(xplot,yplot,colplot):
        ax.plot(xstar/1000, ystar/1000, color=colstar, marker='*', markersize=Marker_Size, markeredgecolor='black', markeredgewidth=1.2)
    ...
    cbar.ax.tick_params(labelsize=18)
    cbar.set_label(r'$\chi$', fontsize=28)
    ...
    ###Force x and y axis to same scale
    ax.set_aspect('equal', adjustable='box')
    ax.set_title('Basal velocity [m/d]', fontsize=21, weight='bold')
    #shading
    ...
    ###Prepare the subplot
    fig5, ax = plt.subplots(figsize=(15, 12), constrained_layout=True)  # , gridspec_kw={'hspace': -0.15})
    ax.set_xlabel(r'X [km]', fontsize=18)
    ax.set_ylabel(r'Y [km]', fontsize=18)
    ax.tick_params(labelsize=16)  # fontsize of the tick labels
    ax.grid(True)
    ...
    ###Force x and y axis to same scale
    ax.set_aspect('equal', adjustable='box')
    ax.set_title(r'$\tau_\mathrm{b}/N$ ratio', fontsize=21, weight='bold')
    #shading
    clevs=np.arange(0.0, 0.501, 0.025)   ## cbar for shading
    vmin, vmax = clevs[0], clevs[-1]
    Colormap = 'YlGnBu_r'
    cmap = cmx.get_cmap(Colormap)
    #colorbar
    ...
    ax.plot(xc/1000, yc/1000 , color='k', linewidth=2)
    # ##Add contour line at taub/N = C
    # CS1_contour = ax.contour(Xbed / 1000, Ybed / 1000, taub_over_N, levels=[C], colors='black', linewidths=2, linestyles='-')
    # # Hatch the area where taub/N > C
    # CS1_hatch = ax.contourf(Xbed / 1000, Ybed / 1000, taub_over_N,levels=[C, np.max(taub_over_N)],colors='none', hatches=['////'], alpha=0)
    ###Plot stars where we plot variable versus time
    for xstar,ystar,colstar in zip(xplot,yplot,colplot):
        ax.plot(xstar/1000, ystar/1000, color=colstar, marker='*', markersize=Marker_Size, markeredgecolor='black', markeredgewidth=1.2)
    ###Below we remove colors that are outside of glacier contour
