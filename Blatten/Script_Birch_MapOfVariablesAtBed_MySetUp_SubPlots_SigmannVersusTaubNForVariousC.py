...
if __name__ == "__main__":
 ...
 #   Timestep_size = 0.01/365 ### In years !!
    ...
    ### At which day do we want the figure to be made : 40 to be sure to have steady state in cavitation
    OutputOfPlot = 40
    ...
    fig1, axes = plt.subplots(nrows=2, ncols=len(Cvalues), figsize=(15, 12), constrained_layout=True, sharex=True, sharey=True)
    for j,C in enumerate(Cvalues):
        ...
        Df_wholebed=Df_wholebed_full[Df_wholebed_full['Count']==OutputOfPlot/(Timestep_size*OutputInterval*365)].copy()
        ...
        ax.set_title('C = {}'.format(C), fontsize=21, weight='bold')
        #shading
        ...
        #### For the steady state of cavitation hatched places where taub/N has reached the threshold C (with a margin of 5%)
        CS1_contour = ax.contour(Xbed / 1000, Ybed / 1000, taub_over_N, levels=[C-0.05*C], colors='black', linewidths=1.2, linestyles='-')
        # # Hatch the area where taub/N => C (with a margin of 6%)
        ...
        ax.plot(xc/1000, yc/1000 , color='k', linewidth=2)
        #### Report contour of areas where taub/N threshold is reached (with margin of 5%)
        CS1_contour = ax.contour(Xbed / 1000, Ybed / 1000, taub_over_N, levels=[C-0.05*C], colors='black', linewidths=1.2, linestyles='-')
        ###Plot stars where we plot variable versus time
        if Stars:
            for xstar,ystar,colstar in zip(xplot,yplot,colplot):
