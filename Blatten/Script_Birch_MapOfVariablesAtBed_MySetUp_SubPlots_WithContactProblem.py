...
as subplots with one column per desired day for the case in which the contact problem is solved
...
if __name__ == "__main__":
    ...
    str_Year = '2011' ###year of interest
 ...
 #   Timestep_size = 0.01/365 ### In years !!
    ...
    OutputOfPlots = [1, 3, 7, 22] ##[0, 1, 3, 7]  ###At which output (count) do we want to make the plot (correspond to the day of simu)
    ...
    if str_Year == '2025':
        ...
        Pathroot_Contour = Path('/home/brondexj/BETTIK/BlattenEventAdri/MyRateAndStateSetup/1_Mesh/MESH{}/.'.format(str_Year))
        Filename_GlacierContour = 'Contour_Birch_{}.dat'.format(str_Year)
    ...
    Filename_SurfGlacierDEM = 'DEM_surf_Corrected.dat'
    ...
    if str_Year == '2025':
        ...
        Pathroot = Path('/home/brondexj/BETTIK/BlattenEventAdri/MyRateAndStateSetup/4_RateAndState_WithDamage_WithContact_{}/GlacierOut/.'.format(str_Year))
        Filename = 'MyBirch{}_RS_C{}_WithD_Cont_SaveScalInt10_tsp001d__bed.dat'.format(str_Year, str_Cvalue)
        # For following restarts: Regex pattern to extract Restart Number and restart position ($ compulsory to avoid the .dat.names)
        pattern = re.compile(r"MyBirch{}_RS_C{}_WithD_Cont_SaveScalInt10_tsp001d_Rstt(\d+)_tsp(\d+)__bed\.dat$".format(str_Year, str_Cvalue))
    ##Load data corresponding to first simu
    Df_wholebed_full = pd.read_csv(Pathroot.joinpath(Filename), names=Col_Name, sep=r'\s+')
    ...
    if str_Year == '2025':
        if C == 0.3:
            ...
            Df_wholebed_full = Df_wholebed_full[Df_wholebed_full['Timestep'] <= 2901].reset_index(drop=True)
    ...
    for Rstt, RsttPos, fname in matches:
        ...
        if str_Year == '2025' and Rstt < matches[-1][0]:  ###This operation is meaningful only if we are not at the last restart
    ...
    if Restart_dfs:  ##Only if Restart_dfs is not empty (there are restarts !)
        Df_wholebed_full = pd.concat([Df_wholebed_full] + Restart_dfs, ignore_index=True)
    ...
    ###Prepare a last figure : subplots with output days as column and theta, taub/N, Sigmann ,and ub as lines
    fig3, axes3 = plt.subplots(nrows=4, ncols=len(OutputOfPlots), figsize=(15, 16), constrained_layout=True, sharex=True, sharey=True)
    ...
    ##############################################################
        ...
        if j == len(OutputOfPlots) - 1:
            ...
            CS1_hatch = ax.contourf(Xbed / 1000, Ybed / 1000, taub_over_N,
                                    levels=[C - 0.05 * C, np.max(taub_over_N)], colors='none', hatches=['////'], alpha=0)
        ### At floating areas (GM <0), make a contour
        CS1_GLcontour = ax.contour(Xbed / 1000, Ybed / 1000, GM, levels=[-0.1], colors='black',linewidths=1.2, linestyles='-')
        # ###Plot stars where we plot variable versus time
        if Stars:
        ...
        ##############################################################
        ###     PLOT MAPS OF VARIABLE OF INTEREST FOR FIGURE 3     ###
        ##############################################################
        ...
        ax.imshow(ortho_crop, extent=extent_km, origin='upper', cmap='gray', zorder=0)
        ###Fills up the map with colors for SigmaEq
        CS1 = ax.contourf(Xbed / 1000, Ybed / 1000, 1 - theta, levels=clevs, cmap=cmap, extend='both', alpha=0.7,
                          antialiased=True)
        # CS1 = ax.pcolormesh(Xbed / 1000, Ybed / 1000, theta, cmap=cmap, shading='auto', vmin=vmin, vmax=vmax, alpha=0.6, zorder=1)
        ### Plot contours of debris thickness
        CS1_debriscontour = ax.contour(Xbed / 1000, Ybed / 1000, Hdebris_smoothed, levels=DebrisContour_Levels,
                                       colors=DebrisContour_Color, linewidths=DebrisContour_Thick, linestyles='-')
        ###Put labels with position determined manually
        ax.clabel(CS1_debriscontour, DebrisContour_Levels, inline=True, inline_spacing=2, fmt="%d",
                  manual=DebrisContour_LabelPositions, fontsize=7.5)
        ax.plot(xc / 1000, yc / 1000, color='k', linewidth=2)
        # ###Plot stars where we plot variable versus time
        if Stars:
            for xstar, ystar, colstar in zip(xplot, yplot, colplot):
        ...
        if j == len(OutputOfPlots) - 1:
            ...
            CS1_hatch = ax.contourf(Xbed / 1000, Ybed / 1000, taub_over_N,
                                    ...
                                    alpha=0)
        ### At floating areas (GM <0), make a contour
        CS1_GLcontour = ax.contour(Xbed / 1000, Ybed / 1000, GM, levels=[-0.1], colors='black', linewidths=1.2,
                                   linestyles='-')
        # ###Plot stars where we plot variable versus time
        if Stars:
            for xstar, ystar, colstar in zip(xplot, yplot, colplot):
        ...
        ax.imshow(ortho_crop, extent=extent_km, origin='upper', cmap='gray', zorder=0)
        ###Fills up the map with colors for SigmaEq
        CS1 = ax.contourf(Xbed / 1000, Ybed / 1000, ub / 365, levels=clevs, cmap=cmap, extend='both', alpha=0.7,
                          antialiased=True)
        # CS1 = ax.pcolormesh(Xbed / 1000, Ybed / 1000, ub/365,  cmap=cmap, shading='auto', vmin=vmin, vmax =vmax ,alpha=0.6,zorder=1)
        ### Plot contours of debris thickness
        CS1_debriscontour = ax.contour(Xbed / 1000, Ybed / 1000, Hdebris_smoothed, levels=DebrisContour_Levels,
